...
export const CommentsSection = ({
  ...
  return (
    <View className="py-4">
      <Text className="text-tertiary text-xl font-bold mb-5">{title} ({comments.length})</Text>
      
      {/* Comment input with improved spacing */}
      <View className="flex-row items-center mb-6">
        <View className="flex-1 flex-row items-center bg-gray-100 rounded-full pr-2">
          <TextInput
            ref={inputRef}
            className="flex-1 py-3 px-5 text-gray-800"
          ...
          >
            <Ionicons name="send" size={18} color="#1E493B" />
          </Pressable>
        </View>
      </View>

      {/* Comments list with better spacing between items */}
      <ScrollView 
